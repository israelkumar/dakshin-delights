...
const Orders: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10 relative">
      ...
      <p className="text-stone-500 mb-10">Manage your recent cravings and track active meals.</p>
      ...
      <div className="space-y-6" aria-busy={loading}>
        ...
        {error && !loading && (
          <div className="text-center py-16">
            ...
            <button onClick={() => window.location.reload()} className="bg-primary text-white px-6 py-2 rounded-lg font-bold">Try Again</button>
          </div>
        )}
          ...
          <>
            {activeOrders.length > 0 && (
              <>
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="text-xl font-semibold uppercase tracking-wider text-primary">Active Orders</h2>
                </div>
                ...
                {activeOrders.map(order => (
                  <div key={order.id} className="bg-white dark:bg-stone-800 border border-primary/20 rounded-xl p-6 shadow-sm" role="article" aria-label={`Active order ${order.id}`}>
                    <div className="flex justify-between items-center mb-6">
                      ...
                      <Link to={`/tracking/${order.id}`} className="bg-primary text-white px-6 py-2 rounded-lg font-bold shadow-lg shadow-primary/20">Track Live</Link>
                    </div>
                    ...
                    <p className="text-sm text-stone-500">{order.status === 'PREPARING' ? 'Preparing your meal' : 'On the way'} {order.eta ? `\u2022 ETA: ${order.eta}` : ''}</p>
                  </div>
                ))}
              </>
            )}
            ...
            {orders.length === 0 && (
              <div className="text-center py-16">
                ...
                <p className="text-stone-500 mb-6">Place your first order from our menu!</p>
                <Link to="/menu" className="bg-primary text-white px-8 py-3 rounded-xl font-bold">Browse Menu</Link>
              </div>
            )}
                    ...
                    <div key={order.id} className="bg-white dark:bg-stone-800 border border-stone-100 dark:border-stone-700 rounded-xl p-5 hover:border-primary/30 transition-all" role="article" aria-label={`Past order ${order.id}, ${order.status}`}>
                      <div className="flex flex-col md:flex-row items-center gap-6">
                        <div className="flex-1 w-full">
                          <div className="flex justify-between items-start mb-2">
                            <div>
                              ...
                              <h4 className="font-bold text-lg">{order.items[0]?.menuItem.name || 'Order'} {order.items.length > 1 ? `& ${order.items.length - 1} more` : ''}</h4>
                            </div>
                        ...
                        <div className="flex gap-3 w-full md:w-auto md:pl-6 md:border-l border-gray-100">
                          <button className="flex-1 md:flex-none whitespace-nowrap px-6 py-2.5 bg-primary/10 text-primary font-bold rounded-lg hover:bg-primary hover:text-white transition-all" aria-label={`Reorder ${order.items[0]?.menuItem.name || 'order'}`}>Reorder</button>
                          <button className="flex-1 md:flex-none whitespace-nowrap px-6 py-2.5 border border-stone-200 text-stone-600 font-medium rounded-lg hover:bg-stone-50 transition-all" aria-label={`View details of order ${order.id}`}>Details</button>
                        </div>
                      </div>
                    </div>
                  ))}
